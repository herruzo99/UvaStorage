...
int main(int argc, char *argv[]) {
	...
	MPI_Datatype MPI_Rectangle, MPI_RectangleExt;
	...
	if ( rank == nprocs-1 ) {
		...
		MPI_Send( array, nprocs, MPI_RectangleExt, 0, tag, MPI_COMM_WORLD );
	...
	if ( rank == 0 ) {
		Rectangle array[nprocs];
		MPI_Status stat;
		MPI_Recv( array, nprocs, MPI_RectangleExt,nprocs-1 , tag, MPI_COMM_WORLD, &stat);
