...
class Princi:
    ...
    def __init__(self):
        ...
        self.glade.add_from_file('Pantalla_minas_copia.glade')
        ...
        self.etiqueta_solucion = self.glade.get_object('solucion_etiqueta')
        ...
        self.boton_solucion = self.glade.get_object('solucion')
    ...
    def pausa(self, widget):
        ...
        if self.pausado:
            ...
            self.boton_solucion.set_sensitive(True)
        ...
        # para el tiempo
            ...
            self.boton_solucion.set_sensitive(False)
    ...
    #función que se pide implementar en la defensa
    #esta función muestra el resultado
    def on_solucion_clicked(self, widget):
        self.submenu_etiquetas.hide()
        self.etiqueta_solucion.show()
        self.jugando=False
        self.boton_pausa.set_sensitive(False)
        for i in range(len(self.matriz)):
            for j in range(len(self.matriz[0])):
                ...
                if self.juego.matriz[i][j].mina:
                    ...
                    img.set_from_pixbuf(self.imgs[0])
                    self.matriz[i][j].remove(self.matriz[i][j].get_children()[0])
                    self.matriz[i][j].add(img)
                self.matriz[i][j].disconnect_by_func(self.boton_mina)
                img.show()
